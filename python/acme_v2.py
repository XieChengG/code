...
class acme_v2:
    ...
    def write_ngin_authx_file(self, auth_to, token, acme_keyauthorization, index):
        ...
        rec = re.compile(r"root\s+([^\n;]*;)\n(?P<other>(\n|.)*?)#CERT-APPLY-CHECK--START(?P<target>(.|\n)*?)#CERT-APPLY-CHECK--END")
        check = rec.search(nginx_conf)

        if check is None:
            root_line = re.search(r"server_name(\n|.)*?root\s+([^\n;]*;)\n", nginx_conf)
            if root_line is None:
                raise Exception("配置文件中找不到root项，无法定位验证文件的位置。")
            ...
            new_conf = re.sub(r"root\s+([^\n;]*;)", root_line.group() + default_cert_apply_check, nginx_conf,1)
